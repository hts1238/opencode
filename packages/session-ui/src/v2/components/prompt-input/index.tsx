...
export function PromptInputV2(props: PromptInputV2Props) {
  ...
  const updateCursor = () => {
    ...
    savedCursorCollapsed = selection.isCollapsed
    if (!savedCursorCollapsed) return
  ...
  const restoreSavedCursor = () => {
    ...
    if (!composing && savedCursorCollapsed) props.controller.restoreCursor()
    return true
  }
  ...
  onMount(() => {
    makeEventListener(document, "selectionchange", () => {
      if (!document.hasFocus() || document.activeElement !== editor) return
      updateCursor()
    })
    makeEventListener(window, "blur", () => {
      if (document.activeElement === editor) restoreCursorOnWindowFocus = true
    })
