...
export function createPromptInputV2Controller(input: {
  ...
  const onKeyDown = (event: KeyboardEvent) => {
    if (
      ...
      !event.shiftKey &&
      event.key === "Enter" &&
      input.view.submit.onForceSubmit
    ) {
      event.preventDefault()
      if (!event.repeat) input.view.submit.onForceSubmit()
      return true
    }
    if (
      state.mode === "normal" &&
  ...
  const restoreCursor = (cursor = draft.state.cursor ?? promptLength(draft.state.prompt)) => {
