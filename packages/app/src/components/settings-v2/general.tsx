import { Component, Show, batch, createMemo, createResource, createSignal } from "solid-js"
...
export const SettingsGeneralV2: Component<{
  ...
  const skillsDirectory = createMemo(() => {
    if (!props.sessionID) return
    return serverSync().session.lineage.peek(props.sessionID)?.session.directory
  })
  const [refreshingSkills, setRefreshingSkills] = createSignal(false)
      ...
      <SettingsListV2>
        ...
        >
          <ButtonV2
            size="normal"
            variant="neutral"
            disabled={refreshingSkills() || !skillsDirectory()}
            onClick={() => void refreshSkills()}
          >
            {language.t(
              refreshingSkills()
                ? "settings.general.skills.refresh.action.refreshing"
                : "settings.general.skills.refresh.action",
            )}
          </ButtonV2>
        </SettingsRowV2>
